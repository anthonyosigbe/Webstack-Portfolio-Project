const getHumanMonth = (m) => {
  const [, month] = new Date(new Date().getFullYear(), m - 1, 1)
    .toDateString()
    .split(" ");
  return month;
};
...
const updateTopMonthsUI = (topMonth, type) => {
  if (type === "expenses") {
    document.querySelector(".expense-top-month").textContent = getHumanMonth(
      parseInt(Object.keys(topMonth)[0])
    );
...
const updateThisMonthUI = (data = [], type = "expenses") => {
  ...
  const currentMonthData = data.find((item) => {
    ...
    return key === currentMonthNumber;
  });

  if (currentMonthData) {
    if (type === "expenses") {
      document.querySelector(".expense-this-month").textContent = getHumanMonth(
        parseInt(Object.keys(currentMonthData)[0])
      );
      document.querySelector(
    ...
    console.error("No data found for the current month");
  }
};
