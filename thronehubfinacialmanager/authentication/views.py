...
class UsernameValidationView(View):
    def post(self, request):
        ...
        if not str(username).isalnum():
            return JsonResponse({'username_error': 'The username should contain only letters and numbers.'}, status=400)
    ...
    def post(self, request):
        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                ...
                messages.success(request, 'Account successfully created, check your provided email for activation link')
                return render(request, 'authentication/register.html')
    ...
    def get(self, request, uidb64, token):
        ...
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                messages.info(
                    request, 'Password link is Invalid, Please request a new password')
                return render(request, 'authentication/reset-password.html')
